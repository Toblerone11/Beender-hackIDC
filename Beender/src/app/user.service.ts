...
export class UserService {
  ...
  getTimeline(user_id:string,timeline_id:number){
    var API = "http://192.168.43.122:8080/users/"+user_id
    return this.http.get(API)
      // (res:Response) => <Timeline>new User(res.json()).timeline[0])

    .map((res:Response) => <Timeline>new User(res.json()).timeline[0]);
